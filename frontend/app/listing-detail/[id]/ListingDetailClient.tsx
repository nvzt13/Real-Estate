...
function ListingDetailPage({ id }: { id: string }) {
  ...
  const [mainImage, setMainImage] = useState<string | undefined>(selectedListing?.images && selectedListing?.images[0]);

  useEffect(() => {
    setMainImage(selectedListing?.images && selectedListing?.images[0]);
  }, [selectedListing]);
      ...
      <Row className="g-4">
        ...
        <Col lg={6}>
          ...
          <h3 className="mt-2" style={{ color: '#5e1ee8' }}>
            ₺{selectedListing.price && selectedListing.price.toLocaleString('tr-TR') ||  0 }
          </h3>

          <h5 className="fw-bold mt-4">Özellikler</h5>
          <Row>
            {selectedListing.category === "ev" && selectedListing.specs ? (
              <>
